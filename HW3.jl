...
student = (name = "Quilee Simeon", kerberos_id = "qsimeon@mit.edu")
...
# ╔═╡ 98101a20-9b57-4b90-9e06-48f5dd413e17
AbstractTrees.children(Real)
...
function count_descendants(T::Type)
	if isconcretetype(T)
		return 1
	else
		count = 1
		for t in children(T)
			count += count_descendants(t)
		end
		return count
	end
end

# ╔═╡ 28673e1d-eb4a-41f5-8732-0d3d27fce75e
count_descendants(Real)
...
# ╔═╡ 7c3239b8-373d-4aba-9dc0-52d7422628b7
# Get the type of the function
typeof(my_identity)

# ╔═╡ c3732ad9-dd87-4676-8539-70f83ce501e6
# Get the supertypes of the function type
...
# ╔═╡ 55885ebd-6c5a-4e8a-bd59-4e50ac828593
begin
	println([1 2; 3 4] .* (6 * ones(2, 2)))
	println([1 2; 3 4] * (6 * ones(2, 2)))
end
...
begin
	...
	function (func::FunctionWrapper)(args...; kwargs...)
...
# ╔═╡ 4397398c-6963-4fad-9aec-7130ec28b566
# md"""
# The function `choice(x)` is type-unstable because the return type depends on the value of `x`, rather than just its type. Specifically:

# - If `x` is less than 0, the function returns a string `"negative number"`.
# - If `x` is greater than or equal to 0, the function returns `x`, which could be any type (e.g., `Int`, `Float64`, etc.).

# Since the return type could either be a `String` or the type of `x`, Julia cannot deduce the return type at compile time. The result is type instability, which makes the code less efficient because Julia has to handle different types at runtime.

# To make the function type-stable, we could modify it to always return the same type, such as converting all results to a string or number.
# """
...
let
	func = FunctionWrapper(x->x^2)
	x = 3.0
	@code_warntype func(x)
end

# ╔═╡ 4a3ececc-1259-46f3-9dbc-afe0459d7cb5
let
	func = FunctionWrapper(x->x^2)
	x = 3.0
	@code_llvm func(x)
...
# ╔═╡ c92ce038-6357-4ebc-afbe-8f21f35f8865
# debugging
let
	typeof(Multiplier(2.0)), fieldnames(Multiplier), fieldtypes(Multiplier)
end
...
# ╔═╡ 92797ee8-0fbf-4f49-ac2a-fcf6353f38cb
# debugging
let
	Base.:*(x::String, m::Int) = repeat(x, m)
	Base.:*(m::Int, x::String) = Base.:*(x::String, m::Int)
end

# ╔═╡ 8ac34de6-b3a6-47b2-a02f-bb66caad8de1
# debugging
# now thte type of mult.a is not forgotten by the GenericMultiplier
let
	a = 2.0
	mult = Multiplier(a)
	x = 1.0
...
# ╔═╡ ae5ed675-0019-4f58-8902-4354311a31e3
# debugging
GenericMultiplier{Matrix{Float64}}([1 2; 3 4])
...
# ╔═╡ 61d37ce4-32eb-4887-8353-bed66678a813
fieldnames(GenericMultiplier{Float64}), fieldtypes(GenericMultiplier{Float64})

# ╔═╡ a6d1777f-b1e8-4255-b87b-56fc8d584021
# debugging
GenericMultiplier{Float64}
...
# ╔═╡ f5d41eed-eb75-40a3-a3ea-e0617e3e130b
bench = @benchmark GenericMultiplier(randn(5,1000))(randn(1000, 3))

# ╔═╡ 135d4a98-b1f7-4bd7-8e1c-ddc0ab7f08fa
# debugging
println(bench.times[1:10]) # measured in nanoseconds

# ╔═╡ d574af8a-cdcf-495b-963e-7b47bd9f188a
@btime GenericMultiplier(randn(5,1000))(randn(1000, 3))

# ╔═╡ 3e3eb272-dd9e-43df-b4c1-969dc3b952f1
# debugging
@belapsed GenericMultiplier(randn(5, 1000))(randn(1000, 3))
...
# ╔═╡ 7db06080-cec5-4855-95cc-2908a4f86513
# debugging
Int <: Signed <: Integer <: Number <: Real  <: Any

# ╔═╡ 78db766d-f773-495c-a675-c5727de039f0
# debugging 
Int64 <: Real

# ╔═╡ 4c20c55a-54c1-45b9-ab91-58203cd42a42
# debugging 
Rational <: Real

# ╔═╡ 0c6b74ef-1344-4e04-be6d-1d46fe8e54bc
# debugging
typeof(1.0 + 0.2im)

# ╔═╡ 995e939c-81aa-4612-a91e-b269320e786f
# debugging
Complex <: Number

# ╔═╡ dcd559fe-a302-4ceb-89ed-febe0d452aaa
# debugging
Real <: Complex

# ╔═╡ 546fd1a2-02f5-465e-adff-defd8bb56cdf
# debugging
Complex <: Real
...
# ╔═╡ f9aaeb90-3f7b-4bae-a719-a9f670f2a7d7
# MatrixMultiplier1 INCORRECT
let
	# Note that Complex <: Real is FALSE 
	# So this should not work if correct behavior
	a = rand([1:10 ...] .+ 0im, 3, 3) # Matrix{Complex{Int64}}
	mult = MatrixMultiplier1(a) 
	# works because of conversion; incorrect behavior!
	
	# @code_warntype mult(1.0)
end

# ╔═╡ eba52c2b-6ada-4a90-9da8-a55aa541f403
# MatrixMultiplier1 BAD
let
	a = rand([1:10...], 2, 2) # Matrix{Int64}
	mult = MatrixMultiplier1(a) 
	x = 1.0
	@code_warntype mult.a * x
end

# ╔═╡ f52f0114-dc08-49c4-97a1-92cddfebd39d
# # MatrixMultiplier2 CORRECT
# let
# 	# Note that Complex <: Real is FALSE 
# 	# So this should not work if correct behavior
# 	a = rand([1:10 ...] .+ 0im, 3, 3) # Matrix{Complex{Int64}}
# 	mult = MatrixMultiplier2(a) 
# 	# does not work; correct behavior!
# end

# ╔═╡ 5f7c2e0d-1a7e-4ad7-86b7-23b0af893c8c
# # MatrixMultiplier2 CORRECT
# let
# 	# Note that Int64 <: Real
# 	# So this should work if correct behavior
# 	a = rand([1:10...], 2, 2) # Matrix{Int64}
# 	mult = MatrixMultiplier2(a) 
# 	# works; correct behavior!

# 	typeof(a), typeof(mult), fieldnames(MatrixMultiplier2), fieldtypes(MatrixMultiplier2)
# end

# ╔═╡ dc9bbe9f-bba9-43ad-8fba-a0fbbd9ba650
# MatrixMultiplier2 GOOD
let
	a = rand([1:10...], 2, 2) # Matrix{Int64}
	mult = MatrixMultiplier2(a) 
	x = 1.0
...
# ╔═╡ f7364d04-b7c3-4341-a745-419a6fadcd24
# # MatrixMultiplier3 CORRECT
# let
# 	# Note that Complex <: Real is FALSE 
# 	# So this should not work if correct behavior
# 	a = rand([1:10 ...] .+ 0im, 3, 3) # Matrix{Complex{Int64}}
# 	mult = MatrixMultiplier3(a) 
# 	# does not work; correct behavior!
# end

# ╔═╡ e6086413-ec55-49df-bc52-234791abe48f
# # MatrixMultiplier3 CORRECT
# let
# 	# Note that Int64 <: Real
# 	# So this should work if correct behavior
...
# 	typeof(a), typeof(mult), fieldnames(MatrixMultiplier3), fieldtypes(MatrixMultiplier3)
# end

# ╔═╡ 3ed148a6-837b-4584-90eb-98bdfc1976c8
# MatrixMultiplier3 GOOD
let
	a = rand([1:10...], 2, 2) # Matrix{Int64}
	mult = MatrixMultiplier3(a) 
	x = 1.0
...
# ╔═╡ 3ac47f99-0cbe-4cdf-9629-07fee4c7b4b6
# MatrixMultiplier4 INCORRECT
let
	# This one is interesting. MatrixMultiplier4 specifies that a<:Matrix{Real}
	# but due to invariance of type parameters in Julia, this will never be true.
	# unless we exactly have that a::Matrix{Real}.
	# Therefore, something that should work but doesn't is this:
	
	# Note that Int64 <: Real
	# So this should work if correct behavior
	a = rand([1:10...], 2, 2) # Matrix{Int64}
	mult = MatrixMultiplier4(a) 
	# doesn't work; incorrect behavior!

	typeof(a), typeof(mult), fieldnames(MatrixMultiplier4), fieldtypes(MatrixMultiplier4)
end

# ╔═╡ 927e87e6-590e-4ffb-8652-0d53b1d69412
# MatrixMultiplier4 BAD
let
	a = Matrix{Real}(rand([1:10...], 2, 2)) # Matrix{Real}
	mult = MatrixMultiplier4(a) 
	x = 1.0
	@code_warntype mult.a * x
end

# ╔═╡ 0231514f-a962-446d-98e5-e4f65fa35730
# # MatrixMultiplier5 CORRECT
# let
# 	# Note that Complex <: Real is FALSE 
# 	# So this should not work if correct behavior
# 	a = rand([1:10 ...] .+ 0im, 3, 3) # Matrix{Complex{Int64}}
# 	mult = MatrixMultiplier5(a) 
# 	# does not work; correct behavior!
# end

# ╔═╡ ed227d02-234b-44d2-a213-70c6e6403f32
# # MatrixMultiplier5 CORRECT
# let
# 	# Note that Int64 <: Real
# 	# So this should work if correct behavior
...
# 	typeof(a), typeof(mult), fieldnames(MatrixMultiplier5), fieldtypes(MatrixMultiplier5)
# end

# ╔═╡ 59ab64f5-fe64-4cb3-a9d9-33a5c4220b49
# MatrixMultiplier5 GOOD
let
	a = rand([1:10...], 2, 2) # Matrix{Real}
	mult = MatrixMultiplier5(a) 
	x = 1.0
	@code_warntype mult.a * x
end

# ╔═╡ 2af71ca0-7539-408c-b269-6c6c55911203
# # MatrixMultiplier6 CORRECT
# let
# 	# Note that Complex <: Real is FALSE 
# 	# So this should not work if correct behavior
# 	a = rand([1:10 ...] .+ 0im, 3, 3) # Matrix{Complex{Int64}}
# 	mult = MatrixMultiplier6(a) 
# 	# does not work; correct behavior!
# end

# ╔═╡ 46dee0b4-ef0f-4994-a3b7-403166f3ba6a
# # MatrixMultiplier6 CORRECT
# let
# 	# Note that Int64 <: Real
# 	# So this should work if correct behavior
...
# 	typeof(a), typeof(mult), fieldnames(MatrixMultiplier6), fieldtypes(MatrixMultiplier6)
# end

# ╔═╡ 3e72d3cd-c4b4-4325-b15a-b844636789e8
# MatrixMultiplier6 GOOD
let
	a = rand([1:10...], 2, 2) # Matrix{Real}
	mult = MatrixMultiplier6(a) 
	x = 1.0
...
begin
	struct Layer{F<:Function, T1<:Real, T2<:Real}
		W::Matrix{T1}
		b::Vector{T2}
		σ::F
	end
...
let 
	...
	layer2 = Layer(W2, b2, σ)
	network = Network([layer1, layer2])
	@code_warntype network(x)
end

# ╔═╡ 04c8f0f1-bb0b-4fde-9f97-232a51053726
# # debugging
# let
# 	W = randn(2,2)
# 	b = zeros(size(W, 1))
...
# Your code here
function meeting_matrix(p::Vector{T}) where T
    n = length(p)
    M = zeros(T, n, n)
    ...
    for i in 1:n
        for j in 1:n
            if i != j
                # Determine the product of (1 - p_k) for k in between i and j
                if i < j
                    M[i, j] = p[i] * prod(1 .- p[i+1:j-1]; init=1) * p[j]
                else
                    M[i, j] = p[j] * prod(1 .- p[j+1:i-1]; init=1) * p[i]
                end
            end
        end
    end
...
# ╔═╡ 7c86bad8-af50-49af-8827-dc1c8409481a
# debugging
let
	@syms p1 p2 p3
	p = [p1, p2, p3]
...
let
	p = collect(1:5) ./ 5
	M = meeting_matrix(p)
	println("Is M(p) symmetric? ", issymmetric(M))
	Symmetric(M, :L) == Symmetric(M, :U)
end
...
Base.size(M::MeetingMatrix) = (length(M.p), length(M.p))

# ╔═╡ 2d3f2591-3d7b-41b6-9129-777168379510
# Define indexing into the matrix
function Base.getindex(M::MeetingMatrix{R}, i::Int, j::Int) where R
    if i == j
        return zero(R)  # M_ii(p) = 0
    elseif i < j
        return M.p[i] * prod(1 .- M.p[i+1:j-1]; init=one(R)) * M.p[j]
    else
        return M[j, i]  # M is symmetric, so use the lower part for upper indices
    end
end
...
# ╔═╡ 65279081-e26a-4e39-a8a1-f9ee426b0a29
# debugging
# These results don't support our intution above! Why?
let
	p = collect(1:100) ./ 100
	i, j = rand(1:100), rand(1:100)

	@btime meeting_matrix($p)[$i, $j]
	@btime MeetingMatrix($p)[$i, $j] # actually faster too?!
end
...
function multiply_row_neighbors_matrix(p, i, x)
    n = length(p)
    
    # Initialize the sum
    result = 0.0

    # Accumulate products for the right side (i+1 to n)
    pbar_right = @views cumprod(1 .- [0, p[(i+1):n-1]...])
	result += @views sum(pbar_right .* x[(i+1):n])

    # Accumulate products for the left side (i-1 to 1)
    pbar_left = @views reverse(cumprod(reverse(1 .- [p[2:(i-1)]..., 0])))
	result += @views sum(pbar_left .* x[1:(i-1)])
...
function multiply_meeting_matrix(p, x)
	# use the fact the M(p)x = D(p)N(p)D(p)x
	Dx = p .* x # D(p)x
	NDx = multiply_neighbors_matrix(p, Dx) # N(p)D(p)x
	DNDx = p .* NDx # D(p)N(p)D(p)x
	return DNDx
end
...
let
	...
	b = rand(4)
	x1 = A \ b
	x2, _ = KrylovKit.linsolve(A_fun, b)
	x1, x2
end
...
# ╔═╡ 5c193925-900b-4308-9655-cb46c364b44f
# debugging
let
    p = collect(1:5) ./ 5
    M = MeetingMatrix(p)
    ...
    println("M[1, 2]: ", M[1, 2])
    println("M[2, 1]: ", M[2, 1])  # Should be the same due to symmetry
end
...
# ╔═╡ a3408d4a-44b4-423a-b116-2db440ab2b93
md"""
_Does this number depend on the packages we have imported?_ — Yes, the number can change if packages define new types that extend `Real` or its subtypes. Since Julia’s type tree is dynamically extended when new packages are loaded, additional types introduced by packages will increase the count of descendants.
"""
	...
	if @isdefined count_descendants
		try
			...
			if check
				...
				almost(md"`count_descendants(Integer)` should be equal to 15")
			end
		catch e
...
# ╔═╡ f68934c9-de33-4b1d-93a6-6c49518ceb85
md"""
The type of the function `my_identity` is a unique singleton type specific to the function, and its supertypes trace back to `Function` and `Any`.
"""
...
md"""
The return type of the function `choice(x)` depends on the _value_ rather than the _type_ input `x`. Therefore, the function is type-unstable.
"""
...
The same principle applies to [arrays and other containers](https://docs.julialang.org/en/v1/manual/performance-tips/#man-performance-abstract-container).
...
# ╔═╡ 72cfa08b-7d18-4c60-bd61-499b5d2465e1
md"""
`@btime` prints the minimum time and memory allocation before returning the value
"""

# ╔═╡ e728c9cc-076f-4b5b-8901-6ff82c2c205a
md"""
...
| --- | --- | --- |
| `MatrixMultiplier1` | no 	| bad  |
| `MatrixMultiplier2` | yes | good |
| `MatrixMultiplier3` | yes | good |
| `MatrixMultiplier4` | no  | bad  |
...
# ╔═╡ b50e58c3-e3cc-434f-963a-d01994d1cc2f
md"""
We also define $M_{ii}(p) = 0$.
As an example, for $n = 5$, we have where we wrote $\bar{p}_i = 1-p_i$ for convenience.
"""

# ╔═╡ 0bf6a155-7364-4e84-a203-174fd6c2a59c
md"""
```math
M(p) = \begin{pmatrix}
...
md"""
The matrix $M(p)$ is symmetric.

What do you mean by how we _express_ it?
"""
...
let
	...
	mem1 = Base.summarysize(M1)
	mem2 = Base.summarysize(M2)

	md"""
	Memory footprint of (full matrix) `M1 = meeting_matrix(p)`: $mem1 bytes.
	
	Memory footprint of (custom MeetingMatrix) `M2 = MeetingMatrix(p)`: $mem2 bytes.
	"""
end

# ╔═╡ aef77331-eaea-4b34-a944-537f1c0b53a0
md"""
**Perhaps Sacrifices for Memory Efficiency?:**
- Time Complexity: While `M2` uses less memory, each element must be computed on demand, which may increase the time required to access elements, especially if the matrix is large.
- Computational Overhead: With `M2`, accessing elements involves recalculating the values, which introduces some computational overhead compared to a fully materialized matrix like `M1`.
"""
...
md"""
- The first term in the products (non-zero) terms is the same for any given row. For row $i$ the first term in the products is $p_i$.

- The last term in the products (non-zero) terms is the same for any given column. For column $j$ the last term in the products is $p_j$.

This structure reflects the fact that, for any two residents $i$ and $j$ to meet, both must be in a good mood, and the intervening residents between them must all be in a bad mood (reflected in the intermediate terms $(1-p_k)$).
"""
...
# ╔═╡ d3eeb817-336f-4b37-afba-557745d912be
md"""
Recall that:
- multiplying by a diagonal [a 0; 0 b] on the right is like elementwise multiplying by the row vector [a b].
- multiplying by a diagonal [a 0; 0 b] on the left is like elementwise multiplying by the column vector [a; b].
"""
...
# ╔═╡ 496c36fe-54b1-4eed-9034-857f66b700f5
md"""
```math
N(p) = \begin{pmatrix}
...
\bar{p}_2 \bar{p}_3 & \bar{p}_3 & 1 & 0 & 1 \\
\bar{p}_2 \bar{p}_3 \bar{p}_4  & \bar{p}_3 \bar{p}_4  & \bar{p}_4 & 1 & 0 \\
\end{pmatrix}
```
.
"""
...
md"""
- Multiple dispatch allows Julia to select the appropriate method for a function based on the types of all its arguments. For the matrix division `\`, different specialized methods are defined for different matrix types like `Matrix`, `Symmetric`, and `Diagonal`.
- **First case (Dense Matrix)**: The generic method for dense matrices (`Matrix{T}`) is selected when `A \ b` is called for a standard matrix.
- **Second case (Symmetric Matrix)**: The specialized method for symmetric matrices (`Symmetric{T}`) is selected. Julia leverages the symmetry to optimize the solve.
- **Third case (Diagonal Matrix)**: The specialized method for diagonal matrices (`Diagonal{T}`) is selected. Solving a system with a diagonal matrix is faster since only the diagonal elements need to be considered.
"""
...
md"""
- Multiple dispatch can choose algorithms based on the matrix type (e.g., `Symmetric`, `Diagonal`), but it does not inherently choose a method based on the size of the matrix. Matrix size is not a type parameter, so solving systems based on size would require explicit checks in the code rather than leveraging multiple dispatch.
"""
...
# ╔═╡ 9fdec21b-7180-42b7-8268-4855c5ec0d72
# debugging
md"""
**Krylov Methods**:
Krylov methods are iterative techniques for solving large systems of linear equations, where directly computing the solution (e.g., using matrix factorizations) is too expensive. Instead of working with the full matrix, these methods build a sequence of approximations based on a "Krylov subspace" — a space spanned by powers of the matrix applied to the initial residual vector. This approach is much faster for large systems since it avoids the need for full matrix storage or factorizations.

**GMRES (Generalized Minimal Residual)**:
GMRES is a specific Krylov method used to solve non-symmetric or non-positive definite systems. It works by minimizing the residual (the error between the current solution and the true solution) over the Krylov subspace at each iteration. GMRES keeps expanding this subspace until the error is small enough. It is particularly useful for complex systems where more basic methods (like conjugate gradient) don't apply.
"""
...
# ╔═╡ c677a8b5-40fd-4b6f-bc7f-7a51253ab467
md"""
```math
\begin{array}{llllllll}
0 =
...
# ╔═╡ 4674d676-5390-4492-853f-027ea586e22a
md"""
```math
\begin{array}{llllllll}
0 =
...
# ╔═╡ bb065aa5-cddf-4b79-86f3-b16a682b5e53
md"""
```math
\begin{array}{llllllll}
0 =
...
md"""
Define $\varepsilon = [\varepsilon_1, \varepsilon_1, \varepsilon_2, \varepsilon_3, \varepsilon_4]^\intercal$, and the vector $c = [1, 2, 2, 2]^\intercal$, then
```math
A = M(\varepsilon) - \operatorname{diagm}(\varepsilon \odot c)
```
where we use $\odot$ to represent elementwise multiplication and $\operatorname{diagm}(x)$ is the operator that creates a diagonal matrix whose diagonal entries are $c$. 

Note that $\operatorname{diagm}(\varepsilon \odot c) = \operatorname{diag}(\varepsilon) \times \operatorname{diagm}(c)$.
"""

# ╔═╡ cf7f778c-ca71-472b-b86c-f81f0c08b2a0
# debugging
let
	@syms ε₁ ε₂ ε₃ ε₄
	ε = [ε₁, ε₂, ε₃, ε₄]
	...
	# A = M - diagm(ε .* c)
	A = M - Diagonal(ε .* c)
	# typeof(M), typeof(Symmetric(M))
end

# ╔═╡ b242c66b-b122-42ea-8bfd-e5298928316f
# debugging
let
	@syms ε₁ ε₂ ε₃ ε₄
	ε = [ε₁, ε₂, ε₃, ε₄]
	c = [1, 2, 2, 2]
	display(diagm(ε .* c))
	display(diagm(ε) * diagm(c))
	display(Diagonal(ε .* c))
end
...
function longwave_matrix(ε::Vector{T}) where T
	n = length(ε)
	c = ones(T, n)
	c[2:end] *= 2
	M = MeetingMatrix(ε) 
...
function LongwaveMatrix(ε::Vector{T}) where T
	n = length(ε)
	c = ones(T, n)
	c[2:end] *= 2
	m1 = MeetingMatrix(ε) 
	m2 = -Diagonal(ε .* c)
	return LazyMatrixSum(m1, m2)
end
...
function multiply_longwave_matrix(ε::Vector{T}, x) where T
	n = length(ε)
	@assert n == length(x)
	c = ones(T, n)
	c[2:end] *= 2
	m1 = multiply_meeting_matrix(ε, x)
	m2 = -ε .* c .* x
	return m1 + m2 
end
...
# Using x -> multiply_longwave_matrix(ε_val, x)
let
	A = x -> multiply_longwave_matrix(ε_val, x)
	b = shortwave_vector(β_val, S₀_val, σ_val)
	x, _ = KrylovKit.linsolve(A, b)
	T = x.^(1/4)
	F = fahrenheit.(T)
	display(F)
end
...
md"""
> The upper layer becomes slightly cooler.
"""
...
md"""
> Based on our model, the data supports anthropogenic modifications of the atmosphere as the more likely cause for global heating out of our two candidate causes.
"""
...
manifest_format = "2.0"
project_hash = "6393733d5919bf4150aa8f3b0a336df06f6b6258"
