...
using LinearAlgebra, Random, Plots, PlutoUI#, MLDatasets
...
let
	dim, attn_dim, mlp_dim, num_heads, n_tokens = 8, 16, 32, 3, 10
	residual_block = AttentionResidual{Float64}(dim, attn_dim, mlp_dim, num_heads)
	X = randn(Float64, n_tokens, dim)  # example input with 10 tokens, each of `dim` dimensions
...
let
	dim, attn_dim, mlp_dim, num_heads, num_layers, n_tokens = 8, 16, 32, 3, 6, 10
...
It turns out the patch embedding is straightforwardly implemented as a strided convolution. However, we will take the more direct and visualizable approach of chopping up an image into patches and linearly projecting the vector that is the flattened patch to the desired dimensionality. 

Remember transformers perform operate on tokens i.e. the perform transfomations of tokens. What we are doing here is essentially *tokenizing* our image data.

"""

# ╔═╡ 403c724c-26bb-457d-a2d3-d32aa5609590
md"""
...
manifest_format = "2.0"
project_hash = "35f41632712b9202b6aece12595e2ba1a0cd0a67"
